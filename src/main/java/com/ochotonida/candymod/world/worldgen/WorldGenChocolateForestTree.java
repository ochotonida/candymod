...
public class WorldGenChocolateForestTree extends WorldGenAbstractTree {


    private static final IBlockState LOG = ModBlocks.WAFER_STICK.getDefaultState();
    private IBlockState leafType;

    public WorldGenChocolateForestTree(boolean notify) {
        super(notify);
    }

    private void setLeavesState(Random rand) {
        int i = rand.nextInt(3);
        leafType = ModBlocks.CHOCOLATE_LEAVES.getDefaultState().withProperty(CHOCOLATE_TYPE, EnumChocolate.byMetadata(i));
                            ...
                            for (int k1 = position.getZ() - l2; k1 <= position.getZ() + l2; ++k1) {
                                ...
                                if (Math.abs(j1) != l2 || Math.abs(l1) != l2 || rand.nextInt(2) != 0 && k2 != 0) {
                                    ...
                                    if (state2.getBlock().isAir(state2, worldIn, blockpos) || state2.getBlock().isAir(state2, worldIn, blockpos)) {
                                        this.setBlockAndNotifyAdequately(worldIn, blockpos, this.leafType);
                                    }
                                }
                            }
                        }
